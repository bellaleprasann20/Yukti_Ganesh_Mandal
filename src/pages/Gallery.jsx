...
// Using YouTube iframe — no file needed, works on all devices.
const YOUTUBE_VIDEO_ID = "IczE6QSPxR8"; // Shorts ID from your link

function VideoModal({ isOpen, onClose }) {
  if (!isOpen) return null;

  // autoplay=1 starts playing when modal opens
  // mute=0 keeps sound on
  // rel=0 hides related videos at end
  const embedUrl = `https://www.youtube.com/embed/${YOUTUBE_VIDEO_ID}?autoplay=1&rel=0&playsinline=1`;

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="absolute inset-0" onClick={onClose} />

      <div className="relative w-full max-w-sm bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-white/10 z-10">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="px-5 pt-4 pb-2">
          <p className="text-white font-bold text-sm">🎉 Festival Highlights</p>
          <p className="text-white/50 text-xs">Yukti Ganesh Mandal, Bhalki</p>
        </div>

        {/* YouTube Shorts aspect ratio = 9:16 (portrait) */}
        <div className="relative w-full bg-black" style={{ paddingTop: "177.77%" }}>
          <iframe
            className="absolute inset-0 w-full h-full"
            src={embedUrl}
            title="Yukti Ganesh Mandal Festival Highlights"
            frameBorder="0"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        >
          {isActive && isPlaying ? <><Pause size={13} /> Pause</> : <><Play size={13} /> Play Track</>}
        </button>
      </div>
    </div>
  );
      ...
      {audioError && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2 mb-3">
          ...
          <p className="text-xs text-red-600 leading-snug">
            ...
            <br />Check file exists in <span className="font-mono font-bold">public/audio/</span>
          </p>
        </div>
      )}
...
export default function Gallery() {
  ...
  const handleOpenVideo = () => {
    audioRef.current?.pause(); // pause music when video opens
    setIsVideoOpen(true);
  };
      ...
      <section className="bg-gradient-to-b from-saffron-50 to-white py-16 px-4 text-center border-b border-gray-100">
        <div className="max-w-2xl mx-auto flex flex-col items-center">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-saffron-500 to-amber-500 hover:from-saffron-600 hover:to-amber-600 text-white font-bold text-sm rounded-xl shadow-md transition-all active:scale-95 flex items-center gap-2"
          >
